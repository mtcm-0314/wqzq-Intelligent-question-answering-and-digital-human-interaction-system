...
def load_prompt_from_file(file_path: Union[str, Path]) -> str:
    """
    Load prompt text from a markdown file.
    It assumes that the first line is a header (starting with '#'),
    and the prompt text is the content following that header.
    """
    # Ensure we work with a Path object
    p = Path(file_path) if isinstance(file_path, str) else file_path

    if not p.exists():
        raise FileNotFoundError(f"File not found: {p}")

    # Read all lines from the file with utf-8 encoding
    ...
    # Join the remaining lines and strip any trailing whitespace
    return "".join(lines).strip()

# Load the prompt text from prompt.md and assign it as the default.
# prompt_str = load_prompt_from_file(r"nonebot_plugin_deepseek\prompt.md")

# prompt_str="""(以下内容进行'深度学习时间序列信号'学术翻译成英文)"""
prompt_str=""""""
...
class CustomModel(BaseModel):
    ...
    api_key: Optional[str] = "sk-aamgdovwgwalykadxfwdkbipuusdggapytopbblgihybnakn"
    """Custom API Key for the model (optional)"""

    prompt: Optional[str] = prompt_str
...
class ScopedConfig(BaseModel):
    api_key: str = "sk-aamgdovwgwalykadxfwdkbipuusdggapytopbblgihybnakn"
    # api_key: str = "sk-35cc9f98c4fd4a26ab1d11e3e2c148cb"
    """Your API Key from deepseek"""
    enable_models: list[CustomModel] = [
        # CustomModel(name="deepseek-chat"),
        # CustomModel(name="deepseek-reasoner"),
        # CustomModel(name="deepseek-ai/DeepSeek-R1-Distill-Llama-8B"),
        # CustomModel(name="deepseek-ai/DeepSeek-R1"),
        CustomModel(name="Pro/deepseek-ai/DeepSeek-V3"),
        
        # CustomModel(name="deepseek-ai/DeepSeek-V3"),
